...
//       // Increased mobile radius multiplier so nodes expand outward on small screens
//       const radius =
//         minDimension < 640 ? minDimension * 0.44 : minDimension * 0.35;
...
//         // Slightly compress the horizontal spread on small screens so nodes don't overflow the edges
//         const radiusX = minDimension < 640 ? radius * 0.9 : radius;
//         const radiusY = radius;

//         const x = cx + radiusX * Math.cos(angle);
//         const yOffset = minDimension < 640 ? Math.sin(angle) * 15 : 0;
//         const y = cy + radiusY * Math.sin(angle) + yOffset;
...
//             <OctagonFrame
//               size={96}
//               className="mx-auto mb-2 transition-transform duration-200 group-hover:scale-105"
//             >
//               {node.image ? (
//                 <img
//                   src={node.image}
...
export default function NetworkHub() {
  ...
  const handleMouseMove = (e) => {
    ...
    const inner = container.querySelector('.hub-parallax-inner');
    if (inner) {
      inner.style.transform = `perspective(1000px) rotateY(${x * 0.015}deg) rotateX(${-y * 0.015}deg)`;
    }
  };
  ...
  return (
    <section className="relative">
      ...
      <div
        ...
        style={{ width: '100%', maxWidth: 1000, height: 700 }}
      >
        {/* Parallax Container Wrapper with original background styles restored */}
        <div
          className="hub-parallax-inner absolute inset-0 w-full h-full transition-transform duration-100 ease-out"
        ...
        >
          ...
          {nodes.map((node, i) => (
            <Link
              ...
              onMouseEnter={() => node.type === 'project' && setActiveIndex(i)}
              onMouseLeave={() =>
                node.type === 'project' && setActiveIndex(null)
              }
            >
              <OctagonFrame
                size={96}
